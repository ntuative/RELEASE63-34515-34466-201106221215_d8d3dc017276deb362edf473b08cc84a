package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1076:int = 3;
      
      private var var_480:AssetLoaderStruct;
      
      private var var_1078:Timer;
      ...
      private const const_2356:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_1078 = new Timer(this.const_2356,1);
         this.var_1078.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1076 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1333);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1076);
            if(this.var_1333.indexOf("?") > 0)
            {
               _loc2_ = this.var_1333 + "&retry=" + this.var_1076;
            }
            else
            {
               _loc2_ = this.var_1333 + "?retry=" + this.var_1076;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2811,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1334[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2041)
         {
            this.var_1332.push([param1,param2]);
            return;
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_864.length > 0 && this.var_865.length < this.const_1947)
         {
            _loc1_ = this.var_864[0];
            _loc1_.startDownloading();
            this.var_865.push(this.var_864.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_864.indexOf(param1) == -1 && this.var_865.indexOf(param1) == -1)
         {
