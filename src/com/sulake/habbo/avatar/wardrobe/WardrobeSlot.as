package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_411:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_411 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this._figure && this.var_1633)
         {
            _loc5_ = this.var_28.manager.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_109,this.var_1093,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_41,parseInt(FigureData.const_849));
         ...
         if(this.var_411)
         {
            if(this.var_411.bitmap)
            {
               this.var_411.bitmap.dispose();
            }
            this.var_411.bitmap = new BitmapData(this.var_411.width,this.var_411.height,true,0);
            _loc7_ = (this.var_411.width - _loc1_.width) / 2;
            _loc8_ = (this.var_411.height - _loc1_.height) / 2;
            this.var_411.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            case "set_button":
               ...
               this.var_1093 = this.var_28.gender;
               this.var_28.handler.saveWardrobeOutfit(this.var_2078,this);
               this.updateView();
               break;
            case "get_button":
               if(this._figure)
               {
                  this.var_28.loadAvatarInEditor(this._figure,this.var_1093,this.var_28.clubMemberLevel);
               }
         }
      }
