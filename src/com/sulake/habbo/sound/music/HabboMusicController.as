package com.sulake.habbo.sound.music
{
   ...
   public class HabboMusicController implements IHabboMusicController, IDisposable
   {
      ...
      private static const const_1666:int = HabboMusicPrioritiesEnum.const_137;
      ...
      private var var_585:IPlayListController = null;
      ...
      public function HabboMusicController(param1:HabboSoundManagerFlash10, param2:IEventDispatcher, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_779.push(new TraxSongInfoMessageEvent(this.onSongInfoMessage));
         this.var_779.push(new UserSongDisksInventoryMessageEvent(this.onSongDiskInventoryMessage));
         for each(_loc5_ in this.var_779)
         {
            this._connection.addMessageEvent(_loc5_);
         }
         this.var_94.addEventListener(RoomEngineSoundMachineEvent.const_1019,this.onJukeboxInit);
         this.var_94.addEventListener(RoomEngineSoundMachineEvent.const_287,this.onJukeboxDispose);
         this.var_94.addEventListener(RoomEngineSoundMachineEvent.const_767,this.onSoundMachineInit);
         this.var_94.addEventListener(RoomEngineSoundMachineEvent.const_796,this.onSoundMachineDispose);
         this.var_1357 = new Timer(1000);
         this.var_1357.start();
         this.var_1357.addEventListener(TimerEvent.TIMER,this.sendNextSongRequestMessage);
         this._events.addEventListener(SoundCompleteEvent.TRAX_SONG_COMPLETE,this.onSongFinishedPlayingEvent);
         _loc6_ = 0;
         while(_loc6_ < HabboMusicPrioritiesEnum.const_267)
         {
            this.var_338[_loc6_] = null;
            this.var_878[_loc6_] = 0;
            _loc6_++;
         }
      }
      ...
      protected function onSongFinishedPlayingEvent(param1:SoundCompleteEvent) : void
      {
         ...
         {
            if(this.getTopRequestPriority() == this._priorityPlaying && this.getSongRequestCountAtPriority(this._priorityPlaying) == this.var_2085)
            {
               this.resetSongStartRequest(this._priorityPlaying);
            }
            _loc2_ = this._priorityPlaying;
            this.playSongWithHighestPriority();
            if(_loc2_ >= HabboMusicPrioritiesEnum.const_433)
            {
               this._events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_124,_loc2_,param1.id,-1));
            }
         }
      }
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            ...
            if(this.var_298)
            {
               ...
               {
                  _loc3_ = this.var_298.getWithIndex(_loc2_) as SongDataEntry;
            ...
            if(this.var_94)
            {
               this.var_94.removeEventListener(RoomEngineSoundMachineEvent.const_1019,this.onJukeboxInit);
               this.var_94.removeEventListener(RoomEngineSoundMachineEvent.const_287,this.onJukeboxDispose);
               this.var_94.removeEventListener(RoomEngineSoundMachineEvent.const_767,this.onSoundMachineInit);
               this.var_94.removeEventListener(RoomEngineSoundMachineEvent.const_796,this.onSoundMachineDispose);
            }
      ...
      private function getSongIdRequestedAtPriority(param1:int) : int
      {
         ...
         if(this.var_338[param1] == null)
         {
            return -1;
         }
         var _loc2_:SongStartRequestData = this.var_338[param1];
      ...
      private function getSongRequestCountAtPriority(param1:int) : int
      {
         ...
         return this.var_878[param1];
      }
      
      private function getTopRequestPriority() : int
      {
         var _loc1_:int = this.var_338.length - 1;
         while(_loc1_ >= 0)
         {
            if(this.var_338[_loc1_] != null)
            {
               return _loc1_;
            }
            _loc1_--;
      ...
      private function getSongDataEntry(param1:int) : SongDataEntry
      {
         ...
         {
            _loc2_ = this.var_298.getValue(param1) as SongDataEntry;
         }
         return _loc2_;
      }
      ...
      public function updateVolume(param1:Number) : void
      {
         ...
         var _loc4_:* = null;
         var _loc2_:int = 0;
         while(_loc2_ < HabboMusicPrioritiesEnum.const_267)
         {
      ...
      {
         if(this.var_701.getValue(param1) == null)
         {
            this.var_701.add(param1,param2);
      ...
      public function samplesUnloaded(param1:Array) : void
      {
         ...
         while(_loc2_ < this.var_298.length)
         {
            ...
            if(_loc3_.id != this.var_1636 && _loc4_ != null && _loc4_.ready)
            {
      ...
      public function get samplesIdsInUse() : Array
      {
         ...
         {
            if(this.var_338[_loc2_] != null)
            {
               _loc3_ = this.var_338[_loc2_];
               _loc4_ = this.var_298.getValue(_loc3_.songId);
               if(_loc4_ != null)
               {
      ...
      private function sendNextSongRequestMessage(param1:TimerEvent) : void
      {
         ...
         this._connection.send(new GetSongInfoMessageComposer(this.var_877));
         Logger.log("Requested song info\'s : " + this.var_877);
         this.var_877 = new Array();
      }
      ...
      private function onSongInfoMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc5_ < _loc4_.length)
         {
            ...
            if(_loc7_)
            {
               ...
               this.var_298.add(_loc6_.id,_loc10_);
               ...
               while(this.var_879.indexOf(_loc6_.id) != -1)
               {
                  this.var_879.splice(this.var_879.indexOf(_loc6_.id),1);
      ...
      {
         if(param2 == -1 || param1 < 0 || param1 >= HabboMusicPrioritiesEnum.const_267)
         {
            return false;
         }
         ...
         this.var_2085 = this.getSongRequestCountAtPriority(param1);
         this.var_1636 = param2;
         if(this._priorityPlaying <= const_1666)
         {
            this.notifySongPlaying(_loc4_);
         }
         if(param1 > HabboMusicPrioritiesEnum.const_137)
         {
            this._events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_121,param1,_loc4_.id,-1));
         }
      ...
      private function notifySongPlaying(param1:SongDataEntry) : void
      {
         ...
         if(param1.length >= 8000 && (this.var_2876 != param1.id || _loc3_ > this.var_2877 + 8000))
         {
            this._soundManager.notifyPlayedSong(param1.name,param1.creator);
            this.var_2876 = param1.id;
            this.var_2877 = _loc3_;
         }
      }
      ...
      private function areSamplesRequested(param1:int) : Boolean
      {
         ...
         return this.var_701.getValue(param1);
      }
      ...
      private function onSongDiskInventoryMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDiskCount)
         {
            ...
            if(this.var_298.getValue(_loc6_) == null)
            {
               this.var_879.push(_loc6_);
      ...
      private function onSoundMachineInit(param1:Event) : void
      {
         ...
         this.var_585 = new SoundMachinePlayListController(this._soundManager,this,this._events,this.var_94,this._connection) as IPlayListController;
      }
      ...
      {
         this.disposeRoomPlaylist();
         this.var_585 = new JukeboxPlayListController(this._soundManager,this,this._events,this._connection) as IPlayListController;
